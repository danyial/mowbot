...
- **After Plan 02 Task 4 (feature + docs commits):** Full SC#1–SC#5 browser walk-through + Foxglove connection test
...
| 3-02-03 | 02 (Commit B) | 2 | VIZ-01, VIZ-03, VIZ-04, VIZ-05 | — | `<ScanOverlay>` implements D-13 memoized Float32Array cartesian + Y-flip, NaN/Infinity skip, 1500 ms stale threshold, 200 ms interval, viridis draw, badge, legend; mounted in `robot-map.tsx`; Foxglove layout JSON valid with /scan + /odometry/filtered + /fix; docs name the "Rosbridge (ROS 1 & 2)" connector (P8) | automated | `cd web && npm run build && grep -q 'STALE_THRESHOLD_MS = 1500' web/components/map/scan-overlay.tsx && grep -q 'useMemo' web/components/map/scan-overlay.tsx && grep -q 'Float32Array' web/components/map/scan-overlay.tsx && grep -q 'py - POINT_SIZE_PX' web/components/map/scan-overlay.tsx && grep -q '<ScanOverlay' web/components/map/robot-map.tsx && python3 -c "import json; json.load(open('web/foxglove/mowerbot.foxglove-layout.json'))" && grep -q "Rosbridge (ROS 1 & 2)" docs/foxglove-integration.md` | ⬜ | ⬜ pending |
| 3-02-04 | 02 (Commit B) | 2 | VIZ-01, VIZ-02, VIZ-03, VIZ-04, VIZ-05 | — | End-to-end SC#1–SC#5 live walkthrough (overlay, CBOR binary frame, stale flip ≤2s both directions, Foxglove 3 panels, viridis + legend) | manual probe | see 03-02-PLAN.md Task 4 — deploy + 5-SC walk | ⬜ | ⬜ pending |
...
| Scan polar overlay renders around robot | VIZ-01, SC#1 | Visual-only behavior | Browser → `/map` → observe 360° overlay pinned to robot marker; asymmetric obstacle probe for Y-flip | 3-02-04 |
| CBOR binary WS frames + bounded buffer | VIZ-02, SC#2 | DevTools runtime inspection over 5 min with 2 tabs | `bufferedAmount` sampled 5× over 5 min, all < 10_000 | 3-02-04 |
| Stale badge flips red ≤1.5 s (worst-case ≤2.0 s) | VIZ-03, SC#3 | Requires live hardware toggle | `ssh pi@10.10.40.23 'docker stop lidar'` with stopwatch | 3-02-04 |
| Foxglove layout opens, 3 panels populate via Rosbridge connector | VIZ-04, SC#4 | External tool behavior | Open `web/foxglove/mowerbot.foxglove-layout.json` in Foxglove Studio → Rosbridge (ROS 1 & 2) → `ws://10.10.40.23:9090` | 3-02-04 |
| Viridis gradient (near=violet, far=yellow) + legend readable | VIZ-05, SC#5 | Visual design judgment | Browser → `/map` → visually confirm gradient direction + bottom-right 0 m → 8 m color bar | 3-02-04 |
